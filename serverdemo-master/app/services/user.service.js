...
service.getTicketInfoUser = getTicketInfoUser;
...
function authenticate(username, password) {
    ...
    db.users.findOne({ username: username }, function (err, user) {
        ...
        if (user && bcrypt.compareSync(password, user.hash)) {
            //vgrif (user && (password, user.password)) {
            // authentication successful
            deferred.resolve({
                _id: user._id,
                username: user.username,
    ...
    db.books.findOne(
        ...
        function (err, ticket) {
            ...
            if (ticket) {
                ...
                deferred.resolve({
                    _id: ticket._id,
                    ticketid:ticket.hardwareid,
                    hardwarename: ticket.hardwarename,
                    hardwaremodel: ticket.hardwaremodel,
...
function getTicketInfoUser(ownername) {
    var deferred = Q.defer();
console.log(ownername);
    

    db.groups.find({ empname:ownername }).toArray(function (err, tickets) {
        if (err) deferred.reject(err.name + ': ' + err.message);
...
function allTic() {
    ...
    db.books.find().sort({ticketid:1}),(function (err, users) {
        if (err) deferred.reject(err.name + ': ' + err.message);
...
function groupone(userParam) {
    ...
    // validation
    db.groups.findOne(
        { hardwarename: userParam.hardwarename },
        function (err, user) {
            if (err) deferred.reject(err.name + ': ' + err.message);
            ...
            if (user) {
                ...
                deferred.reject('Group Name "' + userParam.hardwarename + '" isssssssssss already taken');
           deferred.resolve("Hardware Name is already taken");
...
function update(ownername,status,priority) {
    var deferred = Q.defer();
    db.books.findOne({ hardwareid: ownername }, function (err, user) {
        if (err) deferred.reject(err.name + ': ' + err.message);
         updateUser();
        });
...
function statusUpdate(ownername) {
    var deferred = Q.defer();
console.log("ownername ......");
console.log(ownername);
var _id = ownername;
...
console.log(" -id");
    db.groups.findOne({ _id: mongo.helper.toObjectID(_id) }, function (err, user) {
        if (err) deferred.reject(err.name + ': ' + err.message);
         updateStatus();
        });

    function updateStatus() {
console.log("under updateStatus");
        console.log(_id);
        var set = {
            status: "Assign Ends",
